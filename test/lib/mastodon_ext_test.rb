...
class MastodonExtTest < ActiveSupport::TestCase
  ...
  test "Is public? with public toot" do
    ...
    assert_not status.is_private?
    assert_not status.is_unlisted?
    assert_not status.is_direct?
  ...
  test "Is mention? with mention not starting the toot" do
    ...
    assert_not status_with_mention.is_mention?
  end
  ...
  test "Regression: text content with mention" do
    stub_request(:get, "https://mastodon.xyz/api/v1/statuses/6902726").to_return(web_fixture("mastodon_regression_mention.json"))
    status_with_mention = @client.status(6902726)
